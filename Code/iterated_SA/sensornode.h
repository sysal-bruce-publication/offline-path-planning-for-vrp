...
public:
	 ...
	 *
	 *  Related formula: energy change @f[ \Delta E = \frac{1}{2} * C * V_{max}^2 @f]
	 *
...
public:
	...
	Cluster(Point<T>* p, SensorNode<T>* sn, std::vector<SensorNode<T>> sns);
...
template <class T>
Cluster<T>::Cluster(Point<T>* p, SensorNode<T>* sn, std::vector<SensorNode<T>> sns) {
	this->center = p;
	this->contains.clear();
	this->contains.push_back(sn);

	for (unsigned i = 0; i < sns.size(); i++) {
		double d = this->center->calcDist(sns[i].pos);
		if (d >= MAX_ACOUS_DIST || d <= 0.1) continue;
		else {
			this->contains.push_back(&sns[i]);
		}
	}
}
