/*! @file	main.cpp
*
*  @warning This is the internal main.cpp file of the ODP project.
...
//£¡@def The number of all sensor nodes.
...
*/
vector<SensorNode<float>> readFullData(int sn_num, string fname = "") {
	vector<SensorNode<float>> sn_list;
	sn_list.reserve(sn_num);
...
/*! @fn					vector<Point<float>> readSubPath(int pdv_num, string fname = "") 
*   @brief					Read CSV file of all sensor nodes information.
*   @param pdv_num			The number of PDVs.
*   @param fname				The absolute path to read the CSV file.
*   @exception runtime_error		No such file or directory.
...
vector<Point<float>> readSubPath(int pdv_num, string fname = "") {
	...
	string sub_fname = "../output/sub_path/ga_path" + to_string(pdv_num) + ".csv";
...
/*! @fn	int main()
*   @brief	Implement the main process. Read inputs -> GA -> PDV simulation -> Save data
*/
int main() {
	vector<SensorNode<float>> sn_list;
